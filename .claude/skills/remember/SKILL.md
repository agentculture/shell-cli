...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope shell-cli
...
scope:** the default personal scope is **private** (`--scope shell-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `shell-cli`). Private records are served only to a recall in the same
