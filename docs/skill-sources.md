...
shell-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. shell-cli
...
#   - consumer-identifying prose: `guildmaster` → `shell-cli` (NOT
...
If a re-sync would lose a shell-cli adaptation, lift the change
...
#     → `which shell-cli vendors from guildmaster`
